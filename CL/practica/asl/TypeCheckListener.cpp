...
void TypeCheckListener::enterExprProcCall(AslParser::ExprProcCallContext *ctx) {
	DEBUG_ENTER();
}

void TypeCheckListener::exitExprProcCall(AslParser::ExprProcCallContext *ctx) {

	TypesMgr::TypeId t1 = getTypeDecor(ctx->procedure());
	if(Types.isVoidTy(t1) and !Types.isErrorTy(t1))
	{
		Errors.isNotFunction(ctx);
		auto err = Types.createErrorTy();
		putTypeDecor(ctx, err);
		DEBUG_EXIT();
		return;
	}
...
void TypeCheckListener::exitProcedure(AslParser::ProcedureContext *ctx) {
	...
	auto params = Types.getFuncParamsTypes(t1);
	auto exprParams = ctx->expr();

	auto retType = Types.getFuncReturnType(t1);
	putTypeDecor(ctx, retType);

	if(params.size() != exprParams.size())
	{
	...
	{
		auto exprParamTy = getTypeDecor(exprParams[i]);
		if(not Types.copyableTypes(params[i], exprParamTy) ||
				Types.isErrorTy(exprParamTy))
		{
			Errors.incompatibleParameter(exprParams[i], i + 1, ctx);
		}
	}
...
void TypeCheckListener::exitIdent(AslParser::IdentContext *ctx) {
	...
	if(ctx->expr())
	{
		...
		auto exprType = getTypeDecor(ctx->expr());
		if(!Types.isIntegerTy(exprType))
		{
...
void TypeCheckListener::exitReturnStmt(AslParser::ReturnStmtContext *ctx) {
	...
	auto funcType = Symbols.getCurrentFunctionTy();

	if(!ctx->ret_expr())
	{
		if(!Types.isVoidFunction(funcType))
		{
...
void TypeCheckListener::enterRet_expr(AslParser::Ret_exprContext *ctx) {
	DEBUG_ENTER();
}

void TypeCheckListener::exitRet_expr(AslParser::Ret_exprContext *ctx) {
	auto funcType = Symbols.getCurrentFunctionTy();
	auto retType = getTypeDecor(ctx->expr());
	auto funcRetType = Types.getFuncReturnType(funcType);			

	// std::cout << "Return statement:" << std::endl;
	// std::cout << "Return type:" << std::endl;
	// Types.dump(funcRetType);
	// std::cout << std::endl;
	// std::cout << "Ret_expr type:" << std::endl;
	// Types.dump(retType);
	// std::cout << std::endl;

	if(!Types.copyableTypes(funcRetType, retType))
	{
		Errors.incompatibleReturn(ctx);
	}
